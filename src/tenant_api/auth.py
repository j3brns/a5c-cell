...
from src.tenant_api.constants import PLATFORM_SELF_SERVICE_ADMIN_ROLES, SELF_SERVICE_ADMIN_ROLES
...
def can_manage_tenant_self_service(caller: CallerIdentity, tenant_id: str) -> bool:
    if not is_self_service_admin(caller):
        return False
    if caller.tenant_id == tenant_id:
        return True
    return caller.is_platform_actor and bool(caller.roles & PLATFORM_SELF_SERVICE_ADMIN_ROLES)
