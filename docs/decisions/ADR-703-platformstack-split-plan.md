...
| Compute → AppConfig extension ARN | Required `appConfigExtensionLayerArn` CDK context | Required `appConfigExtensionLayerArn` CDK context | No account-specific layer ARN is embedded in source. Deployments must pass the current AWS-managed ARM64 AppConfig extension layer ARN for the target region. |
